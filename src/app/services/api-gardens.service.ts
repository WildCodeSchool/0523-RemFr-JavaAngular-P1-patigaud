...
export class ApiGardenService {
  ...
  public getGardenList() {
    return this.httpClient
      .get<OldLocationObject>("https://data.tours-metropole.fr/api/records/1.0/search/?dataset=espaces-verts-tours-metropole-val-de-loire&q=&rows=1000&facet=commune&facet=libelle&facet=structure&facet=adresse&facet=aire_en_m")
      .pipe(
        map((response) => {
          return response.records.map((recordLocation: any) => new Location(
            ...
            recordLocation.fields.geo_shape,
          ))
        })
      )
  }
